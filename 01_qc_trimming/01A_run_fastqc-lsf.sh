#!/bin/bash
#BSUB -J 01A_run_fastqc-lsf[1-15]%15  # job name, with array number to run in parallel
...
apptainer run ${FASTQC} fastqc $FASTQ_DIR/${names[${JOBINDEX}]}_*.fastq*
...
mv $FASTQ_DIR/${names[${JOBINDEX}]}_*_fastqc.html $TRIM_DIR
mv $FASTQ_DIR/${names[${JOBINDEX}]}_*_fastqc.zip $TRIM_DIR
